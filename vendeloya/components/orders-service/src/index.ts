...
app.post('/create', verifyToken, async (req: Request, res: Response): Promise<void> => {
  try {
...
// Get order by ID
app.get('/:orderId', verifyToken, async (req: Request, res: Response): Promise<void> => {
  try {
...
app.put('/:orderId/status', verifyToken, async (req: Request, res: Response): Promise<void> => {
  try {
    const { orderId } = req.params;
    const user = (req as any).user;
...
export const ordersService = functions.https.onRequest(app);
