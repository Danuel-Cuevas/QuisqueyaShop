...
app.post('/sales', verifyAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
...
app.post('/inventory', verifyAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    const user = (req as any).user;
    ...
    if (useInventoryCollection && inventorySnapshot) {
      ...
      totalStock = inventory.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0);
      totalReserved = inventory.reduce((sum: number, item: any) => sum + (item.reserved || 0), 0);
      totalAvailable = inventory.reduce((sum: number, item: any) => sum + (item.available || 0), 0);
      ...
      totalStock = products.reduce((sum: number, item: any) => sum + (item.stock || 0), 0);
      totalReserved = 0;
...
// Get report by ID
app.get('/:reportId', verifyAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
...
// List all reports
app.get('/', verifyAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
...
// Delete report
app.delete('/:reportId', verifyAdmin, async (req: Request, res: Response): Promise<void> => {
  try {
    const { reportId } = req.params;
    const reportDoc = await db.collection('reports').doc(reportId).get();
...
export const reportsService = functions.https.onRequest(app);
